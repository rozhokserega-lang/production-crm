-- Дедупликация в web_get_section_articles: «… светло-серый» и «… светло-серый 25»
-- считаются одним изделием при слиянии справочника и план-каталога.

create or replace function public.web_norm_plan_catalog_item(text)
returns text
language sql
immutable
as $$
  select lower(
    regexp_replace(
      regexp_replace(trim(coalesce($1, '')), '\.+\s*$', ''),
      '\s+25(\s*мм)?\s*$',
      '',
      'i'
    )
...
as $function$
  ...
  catalog as (
    select
      ...
      public.web_norm_plan_catalog_item(iam.item_name) as norm_item_name,
  ...
  src as (
    select distinct
      ...
      public.web_norm_plan_catalog_item(pc.item_name) as norm_item_name
  ...
  common_src as (
    ...
    left join public.item_article_map iam
      on public.web_norm_plan_catalog_item(iam.item_name) = s.norm_item_name
  ),
  merged as (

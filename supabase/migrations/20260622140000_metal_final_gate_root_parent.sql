-- Финальная позиция multi_merge: root_parent_id в fork_meta для группировки в UI.
...
set fork_meta = coalesce(mwi.fork_meta, '{}'::jsonb) || jsonb_build_object('root_parent_id', mwi.parent_id)
where mwi.fork_role = 'merge'
  and coalesce(mwi.fork_meta->>'is_final_gate', '') = 'true'
  and mwi.parent_id is not null
  and coalesce(mwi.fork_meta->>'root_parent_id', '') = '';

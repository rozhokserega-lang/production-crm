-- Allow reassigning existing catalog article from "Прочее" to chosen manual section.
-- This covers the workflow where article already exists in generic section and user
-- creates/edits a furniture product that should move into a specific section.
...
begin
  ...
  loop
    ...
    v_can_reassign := trim(coalesce(v_conflict_section, '')) = ''
      or lower(trim(replace(coalesce(v_conflict_section, ''), 'ё', 'е'))) = 'прочее';

    -- Keep strict protection for non-manual catalog rows, except generic "Прочее".
    ...
    -- Reassign manual rows and allowed "Прочее" rows to requested manual section.
    delete from public.item_article_map
      where trim(article) = v_article
        and (
          source = 'manual'
          or trim(coalesce(section_name, '')) = ''
          or lower(trim(replace(coalesce(section_name, ''), 'ё', 'е'))) = 'прочее'
        );

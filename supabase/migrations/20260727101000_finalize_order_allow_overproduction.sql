-- Разрешить указать больше комплектов, чем в плане (перевыпуск на сборке / финале).
...
begin
  ...
  v_ready := greatest(0, coalesce(p_qty_ready, v_total));
  v_debt := greatest(0, v_total - v_ready);

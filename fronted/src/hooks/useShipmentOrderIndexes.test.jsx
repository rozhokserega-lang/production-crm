...
describe("useShipmentOrderIndexes", () => {
  it("builds empty maps when no data provided", () => {
    ...
    expect(result.current.shipmentOrderMaps.byRowWeek.size).toBe(0);
    expect(result.current.shipmentOrderMaps.byItemWeek.size).toBe(0);
  ...
  it("indexes shipment orders by row+week and item+week", () => {
    ...
    expect(result.current.shipmentOrderMaps.byRowWeek.size).toBe(2);
    expect(result.current.shipmentOrderMaps.byItemWeek.size).toBe(2);
  });
  ...
  it("builds orderIndexById from both rows and shipmentOrders", () => {
    ...
    expect(result.current.orderIndexById.get("A-1").name).toBe("Order A");
    expect(result.current.orderIndexById.get("B-1").name).toBe("Order B");
  });
  ...
  it("prefers newer shipment order when merging by row+week", () => {
    ...
    expect(result.current.shipmentOrderMaps.byRowWeek.get(key).qty).toBe(8);
  });
  ...
  it("handles snake_case field names in shipment orders", () => {
    ...
    expect(result.current.shipmentOrderMaps.byRowWeek.size).toBe(1);
  });
  ...
  it("deduplicates orderIndexById by orderId", () => {
    ...
    expect(result.current.orderIndexById.get("A-1").name).toBe("From rows");
  });
  ...
  it("handles order_id (snake_case) in rows", () => {
    ...
    expect(result.current.orderIndexById.get("A-1").name).toBe("Snake case");
  });
});

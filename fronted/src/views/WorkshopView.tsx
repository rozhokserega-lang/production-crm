...
interface WorkshopViewProps {
  ...
  statusClass: (row: WorkshopRow) => string;
  resolveDefaultConsumeSheets: (o: Record<string, unknown>, orders: Record<string, unknown>[]) => number;
  resolveDefaultConsumeSheetsFromBoard: (o: Record<string, unknown>, board: Record<string, unknown>) => number;
  isDone: (s: string | undefined) => boolean;
  isInWork: (s: string | undefined) => boolean;
  isOrderCustomerShipped: (o: WorkshopRow) => boolean;
  actionLoading: string;
  isActionPending: (key: string) => boolean;
  canOperateProduction: boolean;
  runAction: (action: string, orderId: string, payload?: Record<string, unknown>, meta?: Record<string, unknown>) => void;
  executorByOrder: Record<string, string>;
  setExecutorByOrder: React.Dispatch<React.SetStateAction<Record<string, string>>>;
...
export const WorkshopView = memo(function WorkshopView({
  ...
  const prasOptions = Array.isArray(executorOptions?.pras) && executorOptions.pras.length > 0
    ...
    : PRAS_EXECUTORS;
  const isPending = (key: string) => (typeof isActionPending === "function" ? isActionPending(key) : actionLoading === key);
  ...
  return (
    <>
      ...
      {workshopRows.map((o) => {
        ...
        const isPaused = (status: string | undefined) => /пауза/i.test(String(status || ""));

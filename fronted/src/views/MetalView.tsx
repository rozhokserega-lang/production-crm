...
interface MetalViewProps {
  rows: Record<string, unknown>[];
  loading: boolean;
  canOperateProduction: boolean;
  savingKey: string;
  onAdjustStock: (article: string, delta: number, note: string) => Promise<void>;
}

export function MetalView({ rows, loading, canOperateProduction, savingKey, onAdjustStock }: MetalViewProps) {
  const [deltaByArticle, setDeltaByArticle] = useState<Record<string, string>>({});
  ...
  const totals = useMemo(() => {
    return rows.reduce<{ available: number; reserved: number; required: number }>(
      (acc, row) => {
      ...
      <table className="sheet-table">
          ...
          <tbody>
            {sortedRows.map((r) => {
              ...
              const rowStyle: React.CSSProperties = requiredDisplay > 0 ? { background: "#fff5f5" } : canCover ? { background: "#f5fff5" } : {};
              return (
                <tr key={article} style={rowStyle}>
                  <td>{article || "-"}</td>
                  <td>{String(r.metal_name || "-")}</td>

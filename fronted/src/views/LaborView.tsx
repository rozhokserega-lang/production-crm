...
interface PlannerKitRow extends SavedKit {
  kits: number;
  laborPerKitMin: number;
  laborPerKitMinParallel: number;
  totalMin: number;
  totalMinParallel: number;
  hhmm: string;
  hhmmParallel: string;
  missingItems: string[];
}
...
interface LaborViewProps {
  ...
  laborPlannerQtyByGroup: Record<string, string>;
  setLaborPlannerQtyByGroup: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  laborStageTimelineRows: LaborStageTimelineRow[];
  laborSaveSelected: Record<string, boolean>;
  setLaborSaveSelected: React.Dispatch<React.SetStateAction<Record<string, boolean>>>;
...
export const LaborView = memo(function LaborView({
  ...
  const [kitQtyByKey, setKitQtyByKey] = useState<Record<string, string>>({});
  ...
  const [kitBuilderItems, setKitBuilderItems] = useState<KitItem[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const normalized: SavedKit[] = (Array.isArray(rows) ? rows : [])
              ...
              ? (k.items as unknown[])
                  .map((x: unknown) => {
                    if (typeof x === "string") {
                      ...
                      return group ? { group, qty: 1 } as KitItem : null;
                    }
                    const rec = x as Record<string, unknown>;
                    const group = String(rec?.group || "").trim();
                    const qtyRaw = Number(rec?.qty);
                    const qty = Number.isFinite(qtyRaw) && qtyRaw > 0 ? qtyRaw : 1;
                    return group ? { group, qty } as KitItem : null;
                  })
                  .filter(Boolean) as KitItem[]
              : [],
          }))
          .filter((k: SavedKit) => (k.dbId ?? 0) > 0 && k.name && k.items.length > 0);
        setSavedKits(normalized);
      } catch (e: unknown) {
        if (!cancelled && setError) setError(String((e as Error)?.message || e || "Не удалось загрузить комплекты"));
      }
    })();
    return () => {
      cancelled = true;
  ...
  const laborByGroup = useMemo(() => {
    const map = new Map<string, { total: number; pilka: number; kromka: number; pras: number }>();
  ...
  const plannerKitRows = useMemo((): PlannerKitRow[] =>
    savedKits.map((kit) => {
  ...
  const saveKitFromBuilder = () => {
    ...
    const nextKit: SavedKit = { id, dbId: null, dbSaved: false, name, items: [...kitBuilderItems] };
  ...
  const saveKitToDb = async (kit: SavedKit) => {
    if (!callBackend) return;
    ...
    try {
      ...
      const dbId = Number((row as Record<string, unknown>)?.id || 0);
      setSavedKits((prev) =>
        prev.map((x) => (x.id === kit.id ? { ...x, dbId: dbId > 0 ? dbId : x.dbId, dbSaved: dbId > 0 } : x)),
      );
    } catch (e: unknown) {
      if (setError) setError(String((e as Error)?.message || e || "Не удалось сохранить комплект"));
    } finally {
      setKitSavingId("");
    }
  };

  const removeSavedKit = async (kit: SavedKit) => {
    if (kit.dbId && callBackend) {
      setKitDeletingId(kit.id);
      try {
        await callBackend("webDeleteLaborKit", { id: kit.dbId });
      } catch (e: unknown) {
        if (setError) setError(String((e as Error)?.message || e || "Не удалось удалить комплект"));
        setKitDeletingId("");
        return;
      }
      setKitDeletingId("");
                      ...
                      <label style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                            ...
                            setLaborSaveSelected((prev) => ({ ...prev, [r.importKey as string]: checked }));
              ...
              <div style={{ margin: "6px 0 12px" }}>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
                  <label style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      step="1"
                      value={kromkaPosts}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setKromkaPosts(Math.max(1, Number(e.target.value || 1)))}
                      style={{ width: 72 }}
                    />
                  </label>
                  <label style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      step="1"
                      value={prasPosts}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPrasPosts(Math.max(1, Number(e.target.value || 1)))}
                      style={{ width: 72 }}
                    />
                  </label>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 8 }}>
                  <input
                    type="text"
                    value={kitNameDraft}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setKitNameDraft(e.target.value)}
                    placeholder="Имя комплекта (например: Стол + обвязка 1000_80)"
                    style={{ minWidth: 320 }}
                  />
                  <select value={kitItemDraft} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setKitItemDraft(e.target.value)}>
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setKitItemQtyDraft(e.target.value)}
                    placeholder="Кол-во"
                    style={{ width: 96 }}

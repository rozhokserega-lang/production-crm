...
export function isLikelyNetworkError(error: unknown): boolean {
  ...
  if ((error as Record<string, unknown>)?.isNetworkError === true) return true;
...
function normalizeApiError(error: unknown): Error {
  if (isLikelyNetworkError(error)) {
    ...
    Object.assign(wrapped, { isNetworkError: true, cause: error });
    return wrapped;
  }
...
function readStoredSupabaseSession(): SupabaseSession | null {
...
let supabaseAuthSession: SupabaseSession | null = readStoredSupabaseSession();
let supabaseRealtimeClient: ReturnType<typeof createClient> | null = null;

function persistSupabaseSession(session: SupabaseSession | null): void {
...
export function getSupabaseAuthSession(): SupabaseSession | null {
  return supabaseAuthSession;
}

export function getSupabaseAuthUser(): Record<string, unknown> | null {
  return supabaseAuthSession?.user || null;
}

export function getSupabaseRealtimeClient(): ReturnType<typeof createClient> | null {
...
export async function supabaseSignInWithPassword(
  ...
  persistSupabaseSession(payload as SupabaseSession);
  return payload;
}
...
function reportRpcEvent(event: RpcEvent): void {
  const enriched: RpcEvent & { ts: string } = {
    ts: new Date().toISOString(),
    ...event,
  };
  if (typeof window !== "undefined") {
    (window as unknown as Record<string, unknown>).__CRM_RPC_EVENTS__ =
      (window as unknown as Record<string, unknown>).__CRM_RPC_EVENTS__ || ([] as unknown[]);
    ((window as unknown as Record<string, unknown>).__CRM_RPC_EVENTS__ as unknown[]).push(enriched);
  }
  const label = "[CRM RPC]";
  const printer =
...
function buildRpcPayload(
  ...
  if (action === "webPreviewPlansBatch") {
    return {
      p_items: Array.isArray(payload.items)
        ? (payload.items as Array<Record<string, unknown>>).map((x) => ({
  ...
  if (action === "webSetWorkSchedule") {
    return {
      ...
      p_working_days: (() => {
        const raw = payload.workingDays || payload.p_working_days;
        return Array.isArray(raw)
          ? (raw as unknown[]).map((x) => String(x || "").trim().toLowerCase()).filter(Boolean)
          : [];
      })(),
...
export async function supabaseCall(
  ...
  const callWithToken = async (bearerToken: string) => {
...
function isJwtExpiredError(payload: unknown): boolean {

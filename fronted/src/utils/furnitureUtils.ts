...
export function furnitureProductLabel(name: string): string {
...
export function normalizeFurnitureKey(v: string): string {
  return String(v || "")
    .toLowerCase()
    .replace(/[ё]/g, "е")
...
export function normalizeStrapProductKey(v: string): string {
...
export function normalizeDetailPatternKey(v: string): string {
...
export function extractDetailSizeToken(v: string): string {
...
function resolveFurnitureAliasKey(candidates: string[]): string {
  const text = candidates.join(" ");
  const checks: { has: string[]; key: string }[] = [
...
export function parseFurnitureSheet(workbook: XLSX.WorkBook, sheetName: string): ParsedSheet {
...
export function buildFurnitureTemplates(workbook: XLSX.WorkBook, sheetName: string): FurnitureTemplate[] {
  const ws = workbook?.Sheets?.[sheetName];
  if (!ws) return [];
  const rows: unknown[][] = XLSX.utils.sheet_to_json(ws, { header: 1, raw: false, defval: "" });
  const blocks: FurnitureTemplate[] = [];
  let current: FurnitureTemplate | null = null;
  ...
  const uniqueByName = new Map<string, FurnitureTemplate>();
  result.forEach((r) => {
...
export function detailPatternToStrapName(pattern: string): string {
...
export function strapNameToOrderItem(name: string): string {
  const raw = String(name || "").trim();
  if (!raw) return "";
  const sizeMatch = raw.match(/(\d{3,4}_\d{2,3})/);
...
export function isStrapVirtualRowId(rowId: string): boolean {
  return String(rowId || "").startsWith("strap-order:");
}

export function canonicalStrapProductName(name: string): string {
...
export function resolveStrapMaterialByProduct(productName: string): string {
  const key = normalizeStrapProductKey(productName);
  if (key.includes("бел")) return "Белый";
  return "Черный";
}

export function resolveFurnitureTemplateForPreview(

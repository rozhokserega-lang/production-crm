...
// @ts-expect-error — useAppState.js has the implementation, no .d.ts available
  ...
  return (
    <div className="page">
      ...
      <section className="cards">
        {view === "shipment" && (
          <ShipmentView
            ...
            resolvePlanPreviewArticleByName={resolvePlanPreviewArticleByName as unknown as (preview: Record<string, unknown>, lookup: Record<string, unknown>) => string}
        ...
        {view === "metal" && (
          <MetalView
            rows={metalStockRows.filter((row: Record<string, unknown>) => {
        ...
        {view === "admin" && (
          <AdminView
            ...
            formatDateTimeRu={formatDateTimeRu}
            roleOptions={CRM_ROLES as unknown as string[]}
        ...
        {view === "workshop" && (
          <WorkshopView
            ...
            isDone={isDone as (s: string | undefined) => boolean}
            isInWork={isInWork as (s: string | undefined) => boolean}
      ...
      <OrderDrawer
        ...
        getStageLabel={getStageLabel}
        formatDateTimeRu={formatDateTimeRu as (value: unknown) => string}
        isDone={isDone}
        isInWork={isInWork}
        getMaterialLabel={getMaterialLabel}
      ...
      <StrapDialog
        ...
        onPlanWeekChange={(value: string) => setStrapPlanWeek(value.replace(/[^\d-]/g, ""))}
        onDraftValueChange={(name: string, value: string) =>
          setStrapDraft((prev: Record<string, string>) => ({
            ...prev,
        ...
        onClear={() => {
          setStrapItems([]);
          setStrapDraft(strapOptionsForSelectedProduct.reduce((acc: Record<string, string>, name: string) => ({ ...acc, [name]: "" }), {}));
      ...
      <PlanDialog
        ...
        onPlanWeekChange={(value: string) => setPlanWeek(value.replace(/[^\d-]/g, ""))}
        onPlanQtyChange={(value: string) => setPlanQty(value.replace(/[^0-9.,]/g, ""))}

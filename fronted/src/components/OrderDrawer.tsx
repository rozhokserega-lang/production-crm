...
function readAdminComment(row: Record<string, unknown>): string {
  return String(row?.adminComment ?? row?.admin_comment ?? "").trim();
}

function readArticle(row: Record<string, unknown>): string {
  const direct = String(
    row?.product_article ||
      row?.productArticle ||
    row?.article_code ||
...
function splitTitleAndArticle(row: Record<string, unknown>): { title: string; article: string } {
...
interface OrderDrawerProps {
  ...
  formatDateTimeRu: (value: unknown) => string;
  isDone: (status: string) => boolean;
  isInWork: (status: string) => boolean;
  getMaterialLabel: (item: string, material: string) => string;
  canEditAdminComment: boolean;
  onSaveAdminComment?: (comment: string) => Promise<void>;
  savingAdminComment: boolean;
  canAdminStageOverride: boolean;
  onAdminStageOverride?: (orderId: string, stage: string, status: string) => Promise<void>;
...
export function OrderDrawer({
  ...
  const pilkaS = String(first.pilkaStatus ?? first.pilka_status ?? first.pilka ?? "");
  const kromkaS = String(first.kromkaStatus ?? first.kromka_status ?? first.kromka ?? "");
  const prasS = String(first.prasStatus ?? first.pras_status ?? first.pras ?? "");
  const assemblyS = String(first.assemblyStatus ?? first.assembly_status ?? "");
  ...
  const updatedRaw: string =
    lines.reduce((acc: string, row: Record<string, unknown>) => {
      const t = String(row.updatedAt || row.updated_at || row.createdAt || row.created_at || "");
      return t > acc ? t : acc;
    }, "") || String(first.updatedAt || first.updated_at || "");

  const material = first.item
    ? getMaterialLabel(String(first.item || "").trim(), String(first.material || first.colorName || ""))
      ...
      <aside className="order-drawer">
        ...
        <div className="order-drawer__section">
          ...
          <ul className="order-drawer__meta">
            ...
            <li>
              <span>План (первая позиция)</span> <strong>{String(first.week || "—")}</strong>
            </li>
            <li>
                ...
                return (
                  <li key={key} className="order-drawer__line">
                    ...
                    <span className="order-drawer__line-meta">
                      план {String(row.week || "—")} · кол-во {Number(row.qty || 0)}
                    </span>
                  </li>
                );
              })}

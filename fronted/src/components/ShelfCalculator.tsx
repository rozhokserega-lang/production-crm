...
const CATALOG_MAP: Record<string, CatalogItem> = Object.fromEntries(
  CATALOG.furniture_items.map((item) => [normalizeCatalogCode(item.primary.code), item]),
);

function getCatalogItemByCode(code: string): CatalogItem | null {
...
function calcTotals(rows: Row[]): TotalsData {
...
function resolveColor(item: CatalogItem): string {
...
function defaultMaterialByColor(color: string): string {
  if (color === "Вотан") return "Дуб Вотан";
  if (color === "Сонома") return "сонома / бардолино";
  return "Дуб Бардолино";
}

function buildShelfOrderItemName(shelfName: string, materialName: string): string {
...
function parseShelfSize(text: string): { width: number; height: number } | null {
...
function canFitInSheet(piece: { width: number; height: number }, sheetW: number, sheetH: number, cutGap: number): boolean {
...
function fitToRows(piece: { width: number; height: number }, rows: SheetRow[], sheetW: number, cutGap: number): boolean {
  const variants = [
...
function estimateSheets(details: { name: string; qty: number }[], sheetW: number, sheetH: number, cutGap = 3): CutResult {
  const pieces: { width: number; height: number }[] = [];
...
function ShelfBadge({ text, qty }: { text: string; qty: number }) {
  return (
    <span
      style={{
        display: "inline-flex",
...
export default function ShelfCalculator({ canOperateProduction = false, onCreatePlanOrder }: ShelfCalculatorProps) {
  ...
  const [cutResult, setCutResult] = useState<CutResultDisplay | null>(null);
  const [selectedShelves, setSelectedShelves] = useState<SelectedShelf[]>([]);
  ...
  const totalDisplayCards = useMemo((): DisplayCard[] => {
  ...
  const defaultArticleByCardKey = useMemo(() => {
    const byCard = new Map<string, { article: string; score: number }>();
    const byCardArticleSystems = new Map<string, Map<string, number>>();
    activeRows.forEach((r) => {
      ...
      item.pairs.forEach((pair) => {
        ...
        const articleSystems = byCardArticleSystems.get(cardKey) || new Map<string, number>();
        articleSystems.set(article, Number(articleSystems.get(article) || 0) + rowQty);
        byCardArticleSystems.set(cardKey, articleSystems);
      });
    });
    const articleByCard = new Map<string, string>();
    const qrQtyByCard = new Map<string, number>();
    byCard.forEach((v, k) => {
      const article = String(v.article || "").trim();
      articleByCard.set(k, article);
      const articleSystems = byCardArticleSystems.get(k) || new Map<string, number>();
      qrQtyByCard.set(k, Number(articleSystems.get(article) || 0));
    });
    return { articleByCard, qrQtyByCard };
  }, [activeRows]);
  ...
  function runCutEstimate() {
    ...
    setCutResult({ ...result, format, gap, color: cutColor } as CutResultDisplay);
  }

  function toggleShelfCard(card: DisplayCard) {
  ...
  function updateSelectedShelf(id: string, patch: Partial<SelectedShelf>) {

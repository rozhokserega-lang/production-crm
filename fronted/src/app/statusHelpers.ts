...
export function statusClass(order: Record<string, unknown>): string {
  ...
  const inWork = (s: string) => lc(s).includes("в работе");
  const onPause = (s: string) => lc(s).includes("пауза");
...
export function stageBg(stageKey: string, rawBg = "#ffffff"): string {
...
export function getOverallStatusDisplay(order: Record<string, unknown>): string {

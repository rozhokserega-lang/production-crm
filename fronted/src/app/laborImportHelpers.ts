...
function diffMinutesByHms(startRaw: unknown, endRaw: unknown): number {
...
export function parseLaborImportRows(rows: unknown[][] = [], nowKey: number = Date.now()): LaborImportRow[] {
...
export function buildLaborFactPayload(row: Record<string, unknown> = {}): LaborFactPayload {
  return {
    orderId: row.orderId as string | undefined,
    item: row.item as string | undefined,
    week: row.week as string | undefined,
    qty: row.qty as number | undefined,
    pilkaMin: row.pilkaMin as number | undefined,
    kromkaMin: row.kromkaMin as number | undefined,
    prasMin: row.prasMin as number | undefined,
    assemblyMin: row.assemblyMin as number | undefined,
    dateFinished: (row.dateFinished as string | null) || null,
  };
}

export function markLaborImportRowSaved(rows: LaborImportRow[] = [], importKey: string = ""): LaborImportRow[] {
  const key = String(importKey || "");
  if (!key) return rows;
  return rows.map((x) =>
    String(x.import_key || "") === key
      ? { ...x, imported_local: false, imported_saved: true as unknown as undefined }
      : x,
  );
}

export function getLaborImportNoValidRowsError(): string {
  return "Не найдено валидных строк. Используйте экспорт из раздела 'Трудоемкость -> Общая' как шаблон.";
}

export function formatLaborImportError(errMsg: string = ""): string {
  return `Ошибка импорта трудоемкости: ${String(errMsg || "").trim()}`;
}

export function formatLaborSaveRowError(errMsg: string = ""): string {
